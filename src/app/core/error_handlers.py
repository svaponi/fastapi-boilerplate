...
async def _value_error_handler(_: fastapi.Request, exc: ValueError) -> fastapi.responses.JSONResponse:
...
async def _internal_server_error_handler(_: fastapi.Request, exc: Exception) -> fastapi.responses.JSONResponse:
    logging.exception(exc)
    return build_error_response(
...
def setup_error_handlers(app: fastapi.FastAPI):
    ...
    app.add_exception_handler(starlette.exceptions.HTTPException, _internal_http_exception_handler)
